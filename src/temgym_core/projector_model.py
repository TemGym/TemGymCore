...
def _evaluate_simplified_setting(
    ...
    *,
    ...
    if variable_projector_current and u.size < 3:
        raise ValueError("unconstrained_free must have length >= 3 when variable_projector_current=True.")
    if (not variable_projector_current) and u.size < 2:
        raise ValueError("unconstrained_free must have length >= 2.")
    ...
    if variable_projector_current:
        ...
        z_pl1 = float(np.clip(u[2], -60.0, 60.0))
...
def _simplified_setting_residual(
    ...
    *,
    ...
    r_psi = 0.1 * state["psi_total"] / psi_scale
    ...
    pieces: list[np.ndarray] = [
        ...
        weak_pen,
        np.array([i3_low, i3_high, pl1_low, pl1_high], dtype=float),
    ]
...
def solve_simplified_projector_zoom(
    ...
    *,
    ...
    """
    Solve a projector model from target magnifications with zero total image rotation.

    The solver uses two free IL currents per setting and computes the third
    current from the zero-rotation constraint, so `psi_total` is enforced.
    Optionally, PL1 current can be solved as an additional DoF and the
    IL pre-projector front-focal-plane condition can be weighted through
    `ffp_weight`.
    """
    ...
    fit_table: list[dict[str, Any]] = []
    x_prev_il = _bounded_to_unconstrained(np.clip(il_i_ref[:2], i_lo, i_hi), i_lo, i_hi)
    if variable_projector_current:
...
def realize_simplified_projector_setting(
    ...
    *,
    ...
    x0_il = _bounded_to_unconstrained(np.array([i1, i2], dtype=float), i_lo, i_hi)
    if variable_pl1:
